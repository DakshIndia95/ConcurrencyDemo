...
typealias ImageDownloadHandler = (_ image: UIImage?, _ error: ApiError?) -> Void
...
final class ImageDownloadManager {
    ...
    func downloadImage(imgUrl: String, completion: @escaping ImageDownloadHandler) {
        ...
        if let cacheImage = imageCache.object(forKey: url.absoluteString as NSString) {
            ...
            if let operation = (imageDownloadQueue.operations as? [DownloadOperation])?.filter({$0.imageUrl.absoluteString == url.absoluteString && $0.isFinished == false && $0.isExecuting == true}), let operation = operation.first {
                ...
                let operation = DownloadOperation(imageUrl: url)
                operation.queuePriority = .high
                operation.downloadHandler = {[weak self] (img, error) in
                    if let newImage = img {
                        self?.imageCache.setObject(newImage, forKey: url.absoluteString as NSString)
                    }
                    self?.completionHandler?(img, nil)
                }
    ...
    func downloadImageWithLowPriority(imgUrl: String){
        guard let url = URL(string: imgUrl) else {
            completionHandler?(nil, .invalidUrl)
            return
        }
